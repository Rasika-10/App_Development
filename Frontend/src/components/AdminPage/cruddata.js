...
export default function Cruddata() {
  ...
  const deletecontent = (id) => {
    if (window.confirm('Do you want to delete')) {
      axios
        .delete(`http://localhost:8080/candidate/delete/${id}`)
        .then((res) => {
          alert('Record Deleted');
          navigation('/curd');
          window.location.reload(false);
        ...
        <div className='row'>
          <div className='container-data'>
            <div className='card'>
              ...
              <div className='card-body'>
                <Link to='/empadd' className='btn btn-success mb-3' style={{ fontSize: '15px', height: '100%', width: '10%' }}>
                  Add New
                </Link>
                <table className='table table-bordered'>
                  <thead className='bg-primary text-white'>
                    ...
                    {empdata &&
                      empdata.map((item) => (
                        <tr key={item.id}>
                          ...
                          <td>
                            <a onClick={() => loadcontent(item.id)} className='btn btn-secondary m-1' style={{ fontSize: '15px', height: '70%', width: '30%' }}>
                              Edit
                            </a>
